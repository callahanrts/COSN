...
while True:
  ...
  # Switch on commands
  if command == "REGISTER":
    cursor = conn.execute("SELECT * FROM online_users WHERE id = ? LIMIT 1", str(request[1]))
    exists = False
    for row in cursor:
      if row[0]:
        exists = True 
    if exists:
      ...
      server_socket.sendto(pickle.dumps(["ACK", row[0], row[1], row[2]]), address)
    else:
      conn.execute("INSERT INTO online_users(id, ip_address, port) VALUES(?, ?, ?)", (request[1], request[2], request[3]))
      conn.commit()
    ...
    if exists:
      ...
      server_socket.sendto(pickle.dumps(["LOCATION", row[0], row[1], row[2]]), address)
    else:
